...
class ConversationsControllerComprehensiveTest < ActionDispatch::IntegrationTest
  ...
  test "finish conversation by owner" do
    sign_in_as(@user)
    set_tenant(@account)
  ...
  test "finish conversation fails for non-owner" do
    sign_in_as(@user)
    set_tenant(@account)
    ...
    assert_redirected_to conversation_path(conversation)
    assert flash[:alert].present?
    assert conversation.reload.active?  # Not changed
  end

  test "finish requires authentication" do
    set_tenant(@account)
  ...
  # ============================================================================
  # APPROVE SUMMARY ACTION - ALL SCENARIOS
  # ============================================================================

  test "approve_summary with valid params" do
    sign_in_as(@user)
    set_tenant(@account)
    ...
    assert flash[:notice].present?
    # Memory was saved as structured data
    memory_data = conversation.memory_data
    assert_equal "Key decision 1", memory_data["key_decisions"]
  end

  test "approve_summary handles validation errors" do
    sign_in_as(@user)
    set_tenant(@account)

    conversation = @account.conversations.create!(
      title: "Approve Error Test",
      user: @user,
      conversation_type: :adhoc,
      status: :concluding
    )
    ...
    # This might not actually fail with current implementation
    # but let's test the error handling path exists
  ...
  test "approve_summary requires authentication" do
    set_tenant(@account)
  ...
  # ============================================================================
  # REJECT SUMMARY ACTION - ALL SCENARIOS
  # ============================================================================

  test "reject_summary resets conversation to active" do
    sign_in_as(@user)
    set_tenant(@account)
    ...
    assert_nil conversation.context["responded_advisor_ids"]
  end

  test "reject_summary requires authentication" do
    set_tenant(@account)
  ...
  # ============================================================================
  # REGENERATE SUMMARY ACTION - ALL SCENARIOS
  # ============================================================================

  test "regenerate_summary when concluding" do
    sign_in_as(@user)
    set_tenant(@account)
    ...
    conversation = @account.conversations.create!(
      ...
      conversation_type: :adhoc,
      status: :concluding,
      draft_memory: { "old" => "data" }.to_json
    )
    conversation.conversation_participants.create!(advisor: @advisor1, role: :advisor, position: 0)

    assert_enqueued_with(job: GenerateConversationSummaryJob) do
      post regenerate_summary_conversation_path(conversation)
    end
  ...
  test "regenerate_summary fails when not concluding" do
    sign_in_as(@user)
    set_tenant(@account)
  ...
  test "regenerate_summary requires authentication" do
    set_tenant(@account)
  ...
  # ============================================================================
  # CANCEL PENDING ACTION - ALL SCENARIOS
  # ============================================================================

  test "cancel_pending by conversation owner" do
    sign_in_as(@user)
    set_tenant(@account)
    ...
    assert_redirected_to conversation_path(conversation)
    assert flash[:notice].present?
    assert conversation.messages.where(status: :pending).none?
  end

  test "cancel_pending fails for non-owner" do
    sign_in_as(@user)
    set_tenant(@account)
    ...
    assert_redirected_to conversation_path(conversation)
    assert flash[:alert].present?
    assert pending_msg.reload.pending?  # Not cancelled
  end

  test "cancel_pending fails when conversation not active" do
    sign_in_as(@user)
    set_tenant(@account)
  ...
  test "cancel_pending requires authentication" do
    set_tenant(@account)
