...
class AdvisorComprehensiveTest < ActiveSupport::TestCase
  ...
  test "global scope returns only global advisors" do
    ...
    globals = Advisor.global
    assert_includes globals, global_advisor
    assert_not_includes globals, custom_advisor
  end
  ...
  test "custom scope returns only non-global advisors" do
    ...
    customs = Advisor.custom
    assert_includes customs, custom_advisor
    assert_not_includes customs, global_advisor
  end

  test "for_space scope returns advisors for specific space" do
    other_space = @account.spaces.create!(name: "Other Space")

    space_advisor = @account.advisors.create!(
      name: "Space Advisor",
      system_prompt: "You are in space.",
      space: @space,
      llm_model: @llm_model
    )
    other_space_advisor = @account.advisors.create!(
      name: "Other Space Advisor",
      system_prompt: "You are in other space.",
      space: other_space,
      llm_model: @llm_model
    )

    for_space = Advisor.for_space(@space)
    assert_includes for_space, space_advisor
    assert_not_includes for_space, other_space_advisor
  end
  ...
  test "non_scribes scope returns only non-scribes" do
    ...
    non_scribes = Advisor.non_scribes
