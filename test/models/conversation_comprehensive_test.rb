...
class ConversationComprehensiveTest < ActiveSupport::TestCase
  ...
  test "mark_advisor_responded adds advisor to responded list" do
    conv = @account.conversations.create!(
      title: "Test",
      user: @user,
      council: @council
    ...
    conv.mark_advisor_responded(1)
    assert_equal [ "1" ], conv.reload.context["responded_advisor_ids"]

    conv.mark_advisor_responded(2)
    assert_equal [ "1", "2" ], conv.reload.context["responded_advisor_ids"]
  ...
  test "advisor_has_responded? checks if advisor responded" do
    conv = @account.conversations.create!(
      title: "Test",
      user: @user,
      council: @council,
      context: { "responded_advisor_ids" => [ "1", "2" ] }
    )
    conv.conversation_participants.create!(advisor: @advisor1, role: :advisor, position: 0)

    assert conv.advisor_has_responded?(1)
    assert conv.advisor_has_responded?(2)
    assert conv.advisor_has_responded?("1")  # String should also work
    assert_not conv.advisor_has_responded?(3)
    assert_not conv.advisor_has_responded?(nil)
  end

  test "advisor_has_responded? handles empty context" do
    conv = @account.conversations.create!(
      title: "Test",
      user: @user,
      council: @council,
    ...
    assert_not conv.advisor_has_responded?(1)
  end

  test "all_advisors_responded? returns true when all responded" do
    conv = @account.conversations.create!(
      title: "Test",
      user: @user,
      council: @council
    ...
    conv.update!(context: { "responded_advisor_ids" => [ @advisor1.id.to_s, @advisor2.id.to_s ] })

    assert conv.all_advisors_responded?
  end

  test "all_advisors_responded? returns false when not all responded" do
    conv = @account.conversations.create!(
      title: "Test",
      user: @user,
      council: @council
    ...
    conv.update!(context: { "responded_advisor_ids" => [ @advisor1.id.to_s ] })

    assert_not conv.all_advisors_responded?
  end

  test "all_advisors_responded? handles empty responded list" do
    conv = @account.conversations.create!(
      title: "Test",
      user: @user,
      council: @council
  ...
  test "mark_advisor_spoken updates last_advisor_id in context" do
    conv = @account.conversations.create!(
      title: "Test",
      user: @user,
      council: @council
    ...
    conv.mark_advisor_spoken(42)
    assert_equal 42, conv.reload.context["last_advisor_id"]

    conv.mark_advisor_spoken(99)
    assert_equal 99, conv.reload.context["last_advisor_id"]
  end
  ...
  test "last_advisor_id reads from context" do
    conv = @account.conversations.create!(
      ...
      user: @user,
      council: @council,
      context: { "last_advisor_id" => 123 }
    )
    conv.conversation_participants.create!(advisor: @advisor1, role: :advisor, position: 0)

    assert_equal 123, conv.last_advisor_id
  end

  test "last_advisor_id returns nil when not set" do
    conv = @account.conversations.create!(
      title: "Test",
      user: @user,
      council: @council
    ...
    assert_nil conv.last_advisor_id
  end
  ...
  test "council_meetings scope filters by type" do
    adhoc_conv = @account.conversations.create!(
      title: "Adhoc",
      user: @user,
      conversation_type: :adhoc
    ...
    meetings = Conversation.council_meetings.to_a
    assert_includes meetings, council_conv
    assert_not_includes meetings, adhoc_conv
  end
end
